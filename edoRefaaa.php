<?php
	...
	$query01="SELECT 
			 ...
			 IF ((SELECT status 
			 		FROM garantia_liquida 
						WHERE garantia_liquida.credito_id=credito.id)='VIGENTE',(SELECT garantia_liquida.monto_total 
																					FROM garantia_liquida 
																						WHERE garantia_liquida.credito_id=credito.id),0) AS garantia_liquida,
			 cliente.id,
			 cliente.nombre,
			 cliente.apellidos,
			 cliente.rfc,
	...
	$query04="SELECT sum(monto) as capital 
					FROM abono 
						WHERE tipo_pago='0' and pago_id='".$id."'";
	...
	$query05="SELECT sum(monto) as interes 
				FROM abono 
					WHERE tipo_pago='1' and pago_id='".$id."'";
	...
	$queryAbonosP = "SELECT sum(a.monto) as monto 
	                      FROM abono as a 
	                             WHERE a.pago_id=$id AND a.tipo_pago='0'";
	...
	$queryAbonosI = "SELECT sum(monto) as mont 
	                                          FROM abono 
											             where pago_id='".$id."' and tipo_pago='1' ";
	...
	*/
	...
	$ts_sll_es=$tasa/100;
	$cpt_sll_es=($ca_sll[0]+$ca_sll[1]+$ca_sll[2]+$ca_sll[3]);
	$itr_sll_es=($cp_sll[0]+$cp_sll[1]+$cp_sll[2]+$cp_sll[3]);
	/////////////////
	/////1RA AMORTIZACION
	IF ($es_sll[0]=='VIGENTE'){
		$capvencido00=0;
...
function Tabla_6($header01)
	...
	INNER JOIN  credito c on c.id = p.credito_id  AND c.folio='".$Folio."') X";
		...
		{	
			...
			$this->SetX(80);
			$this->Cell(45,4,$row041['COMPORTAMIENTO'],0,0,"C");
			$this->SetX(126);
			$this->Cell(15,4,$row041['DIAS'],0,0,"C");
				  $this->SetX(115);
				  
			$this->Cell(90,4,$row041['DICTAMEN'],0,1,"C");
	...
	while($row032=mysqli_fetch_array($result0312))
	{
		...
		$this->SetX(10);
		$this->Cell(30,5,''.$I.'',0,0,"L");
		$this->SetX(5);
		$this->Cell(45,4,$row032['fecha_pago'],0,0,"C");
		$this->SetX(40);
		$this->Cell(15,4,$row032['monto_capital'],0,0,"C");
		$this->SetX(25);
		$this->Cell(90,4,$row032['monto_intereses'],0,1,"C");
...
*/	$pdf->SetY(120);
	...
	*/
	...
	$pdf->SetY(75);
	$pdf->SetX(10);
	$pdf->Multicell(7,4,'F/P',0,'C');
	$pdf->SetY(75);
	$pdf->SetX(17);
	$pdf->Multicell(22,4,'FECHA PAGO',0,'C');
	$pdf->SetY(75);
	$pdf->SetX(35);
	$pdf->Multicell(25,4,'MONTO',0,'C');
	$pdf->SetY(75);
	$pdf->SetX(60);
	$pdf->Multicell(20,4,'INTERES',0,'C');
	$pdf->SetY(75);
	$pdf->SetX(90);
	$pdf->Multicell(32,4,'COMPORTAMIENTOS',0,'C');
	$pdf->SetY(75);
	$pdf->SetX(116);
	$pdf->Multicell(25,4,'N/DIAS',0,'R');
	$pdf->SetY(75);
	$pdf->SetX(140);
	$pdf->Multicell(30,4,'DICTAMEN',0,'C');
	$pdf->SetY(75);
	$pdf->SetX(184);
	$pdf->Multicell(24,4,'TIPO CLIENTE',0,'R');
